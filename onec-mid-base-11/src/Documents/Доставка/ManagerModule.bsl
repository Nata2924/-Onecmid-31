и ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// Договор (docx)
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Документ.Доставка";
...
Функция ПолучитьДанныеДокументов(МассивОбъектов)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Доставка.Ответственный КАК Ответственный,
	               ...
	               |	Доставка.Контрагент.ИНН КАК КонтрагентИНН,
	               |	Доставка.Контрагент.КПП КАК КонтрагентКПП,
	               |	РеализацияТоваровУслуг.КонтактноеЛицо КАК КонтактноеЛицо,
	               |	РеализацияТоваровУслуг.Контрагент.ЮридическийАдрес КАК КонтрагентЮридическийАдрес,
	               |	Доставка.Контрагент.ЮридическийАдрес КАК КонтрагентЮридическийАдрес
	               |ИЗ
	               |	Документ.Доставка КАК Доставка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	               |		ПО Доставка.Основание = РеализацияТоваровУслуг.Ссылка
	               |ГДЕ
...
Функция ПечатьДоговораНаДоставку(МассивОбъектов)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.Доставка.ПФ_DOC_ДоговорНаДоставку");
